...
def upload_file():
    if request.method == 'POST':
        ...
        return "Please use the form to submit a picture"

@app.route('/upload/therm', methods=['POST'])
def upload_therm():
    therm_string = request.data.decode("utf-8")[1:-1].split(', ')
    therm_matrix_flat = np.array([float(val) for val in therm_string])
    img = therm_matrix_flat.reshape((640, 480))

    img_small = cv2.resize(img, (60,60))
    img_matlab = matlab.single(img_small.tolist())

    img_matlab_q = eng.simpleDRQ(img_matlab)

    img_matlab_qs = matlab.single(img_matlab_q)

    label = int(eng.testingasample(img_matlab_qs,'example__pretrained_network',0))
...
def upload():
    ...
    pixels = labelmap.flatten().tolist()
...
def inference_on_whole_image(img, model):
    ...
    for i in range(nh):
        for j in range(nw):
            ...
            pred = softmax(pred).squeeze().cpu().detach().numpy().transpose(1,
                                                                          2,0)
